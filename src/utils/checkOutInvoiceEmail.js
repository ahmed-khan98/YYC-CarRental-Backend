...
export function buildCheckOutInvoiceMailOptions({
  ...
  const totalLabel = formatMoney(
    computeBillSummary(booking.billEntries ?? []).totalBill ?? booking.totalAmount,
  );

  const text = [
    "Check-out is complete. Thank you for choosing YYC Car Rental.",
    "",
    ...
    "",
    "Your check-out invoice is attached as a PDF.",
    "",
    "YYC Car Rental",
  ].join("\n");

  const html = `
    <p>Check-out is complete. Thank you for choosing YYC Car Rental.</p>
    <p>
    ...
    <p>Your check-out invoice is attached as a PDF.</p>
    <p>YYC Car Rental</p>
  `;
  ...
  return {
    ...
    subject: `Check-Out Invoice ${bookingRef} — YYC Car Rental`,
    text,
    html,
    attachments: [
      {
        filename: `invoice-checkout-${bookingRef}.pdf`,
        content: pdfBuffer,
...
export async function sendCheckOutInvoiceEmail(booking, pdfBuffer) {
  const bookingId = booking?._id;
  try {
    if (!pdfBuffer || !Buffer.isBuffer(pdfBuffer) || pdfBuffer.length === 0) {
      console.info("Check-out invoice email skipped: missing PDF", {
        bookingId: String(bookingId),
      });
    ...
    const mailOptions = buildCheckOutInvoiceMailOptions({
      booking,
      customer,
      car,
      pdfBuffer,
    ...
    console.info("Check-out invoice email sent:", {
      bookingId: String(bookingId),
      to: mailOptions.to,
